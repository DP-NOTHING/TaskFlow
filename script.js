...
const addTask = (isDone) => {
    ...
    taskst.push({task, isDone});
    localStorage.setItem("savedTasks", JSON.stringify(taskst));
    updateView();
...
const editTask = (id) => {
    ...
    const taskText = taskst[taskIndex].task;
    taskst.splice(taskIndex, 1);
    localStorage.setItem("savedTasks", JSON.stringify(taskst));
    updateView();
...
const doTask = (id) => {
    ...
    const taskIndex = parseInt(id[0]);
    taskst[taskIndex].isDone = !taskst[taskIndex].isDone;
    localStorage.setItem("savedTasks", JSON.stringify(taskst));
    updateView();
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if(savedTasks !== null) taskst = [...savedTasks];
    updateView();
})
